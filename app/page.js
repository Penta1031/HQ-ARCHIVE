...
import {
  ...
  ArrowUpDown, ExternalLink, Home, Languages, ListTree, LockKeyhole, Medal, Pencil, Play, Plus, RefreshCw, Search, Share2, Shuffle, Trash2, Trophy, Video, X
} from "lucide-react";
import { adminService, analyticsService, appTabService, archiveIndexService, archiveService, keywordService, recommendedVideoService, tweetMediaService, worldcupService } from "../lib/services";
...
function HomeView({ language = "ko", initialKeyword, onKeywordConsumed }) {
  ...
  useEffect(() => {
    ...
    archiveService.today({ monthDay: requestedDate.slice(5), page: 1, limit: 30 }).then((result) => { setTodayItems(result.items.filter((item) => item.date !== requestedDate)); setTodayPage(1); setTodayHasMore(result.items.length === 30 && result.total > result.items.length); }).catch(() => { setTodayItems([]); setTodayHasMore(false); }).finally(() => { setTodayLoading(false); setTodayLoadedDate(requestedDate); });
  ...
  return (
    <div>
      ...
      {subTab === "today" && <TodayView items={todayItems} loading={todayLoading} loadingMore={todayLoadingMore} hasMore={todayHasMore} onLoadMore={loadMoreToday} todayDate={todayDate} liveTodayDate={liveTodayDate} onDateChange={setTodayDate} language={language} />}
...
function TodayView({ items, loading = false, loadingMore = false, hasMore = false, onLoadMore, todayDate = "", liveTodayDate = "", onDateChange, language = "ko" }) {
  ...
  return <div className="px-4 pt-5"><div className="flex items-end gap-2 rounded-2xl border border-white/10 bg-neutral-900/70 p-3"><button type="button" onClick={()=>moveDay(-1)} aria-label="이전 날짜" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/5 text-neutral-400"><ChevronLeft size={17}/></button><div className="grid min-w-0 flex-1 grid-cols-2 gap-2"><AdminSelect label="월" value={String(month)} onChange={(value)=>setMonthDay(Number(value),day)} options={Array.from({length:12},(_,index)=>String(index+1))} optionLabels={Object.fromEntries(Array.from({length:12},(_,index)=>[String(index+1),`${index+1}월`]))}/><AdminSelect label="일" value={String(day)} onChange={(value)=>setMonthDay(month,Number(value))} options={Array.from({length:daysInMonth},(_,index)=>String(index+1))} optionLabels={Object.fromEntries(Array.from({length:daysInMonth},(_,index)=>[String(index+1),`${index+1}일`]))}/></div><button type="button" onClick={()=>moveDay(1)} aria-label="다음 날짜" className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/5 text-neutral-400"><ChevronRight size={17}/></button><button type="button" onClick={()=>onDateChange?.(liveTodayDate)} disabled={todayDate===liveTodayDate} className="h-10 shrink-0 rounded-xl bg-white px-3 text-[10px] font-black text-black disabled:bg-white/5 disabled:text-neutral-600">오늘</button></div><div className="relative mt-3 overflow-hidden rounded-3xl bg-accent p-5"><div className="absolute -right-6 -top-8 text-[100px] font-black text-white/10">{day || ""}</div><p className="text-xs font-bold text-white/70">ON THIS DAY</p><h2 className="mt-2 text-2xl font-black">{todayDate ? archiveText(language, "todayTitle", { month, day }) : archiveText(language, "today")}</h2></div>
    ...
    {!loading && hasMore && <button type="button" disabled={loadingMore} onClick={onLoadMore} className="mt-5 w-full rounded-xl border border-white/10 bg-neutral-900 py-3 text-xs font-black text-neutral-400 disabled:opacity-50">{loadingMore ? "불러오는 중…" : "30개 더 보기"}</button>}
  </div>;
}
...
function AdminHub({ onClose, tabVisibility, onTabVisibilityChange }) {
  ...
  const sectionTitle = { hub: "관리자 모드", tabs: "탭 노출 관리", archive: "아카이브 관리", index: "인덱스 관리", recommended: "추천 영상 관리", stats: "통계" }[section] || "관리자 모드";
  ...
  return <div className="absolute inset-0 z-[80] flex items-end bg-black/80 p-3 backdrop-blur-md" onClick={onClose}>
    <div onClick={(event) => event.stopPropagation()} className="max-h-[92dvh] w-full overflow-y-auto rounded-[28px] border border-white/10 bg-neutral-950 p-5 shadow-2xl no-scrollbar animate-pop-in">
      ...
      </div> : section === "tabs" ? <TabVisibilityAdmin onBack={() => setSection("hub")} tabVisibility={tabVisibility} onChange={onTabVisibilityChange}/> : section === "index" ? <ArchiveIndexAdmin onBack={() => setSection("hub")}/> : section === "recommended" ? <RecommendedVideosAdmin onBack={() => setSection("hub")}/> : section === "stats" ? <StatsAdmin onBack={() => setSection("hub")}/> : <ArchiveAdmin onBack={() => setSection("hub")}/>}
    </div>
  </div>;
}
...
function StatsAdmin({ onBack }) {
  ...
  const [viewRange, setViewRange] = useState({ from: today, to: today });
  const [searchRange, setSearchRange] = useState({ from: today, to: today });
  ...
  return <div className="py-5">
    ...
    <div className={cn("transition-opacity", loading && "opacity-45")}>
      {section === "views" ? <>
        ...
        <section className="mt-5"><div className="mb-2 flex items-end"><div><h4 className="text-sm font-black">검색어별 누적 검색</h4><p className="mt-1 text-[9px] font-bold text-neutral-600">{searchRange.from === searchRange.to ? `${searchRange.from} 일자` : `${searchRange.from} ~ ${searchRange.to}`} · 중복 검색어 합산</p></div><select value={searchLimit} onChange={(event) => setSearchLimit(Number(event.target.value))} className="ml-auto rounded-lg border border-white/10 bg-neutral-900 px-2 py-1.5 text-[10px] font-black"><option value={5}>5개</option><option value={10}>10개</option><option value={20}>20개</option><option value={50}>50개</option><option value={100}>100개</option></select></div><div className="space-y-2">{searchStats.top.length ? searchStats.top.map((item,index) => <div key={item.query_text} className="flex items-center rounded-xl border border-white/10 bg-white/[.03] p-3"><span className="w-7 text-[10px] font-black text-neutral-600">{index+1}</span><strong className="min-w-0 flex-1 truncate text-xs">{item.query_text}</strong><span className="text-xs font-black text-accent">{formatNumber(item.searches)}회</span></div>) : <div className="rounded-2xl border border-dashed border-white/10 py-10 text-center text-xs font-bold text-neutral-600">선택 기간의 검색 기록이 없습니다.</div>}</div></section>
      </>}
    </div>
  </div>;
}
...
function ArchiveIndexAdmin({ onBack }) {
  const [data, setData] = useState({ categories: [], keywords: [] });
  const [tab, setTab] = useState("category");
  const [form, setForm] = useState({ id: "", name: "", categoryId: "", sortOrder: 0 });
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  const load = async () => {
  ...
  const changeTab = (nextTab) => { setTab(nextTab); setMessage(""); reset(nextTab); };
  const rows = tab === "category" ? data.categories : tab === "keyword" ? data.keywords : data.categories.flatMap((category) => category.subcategories.map((item) => ({ ...item, categoryName: category.name })));
  const edit = (item) => setForm({ id: String(item.id), name: item.name, categoryId: String(item.categoryId || ""), sortOrder: item.sortOrder || 0 });
  const save = async (event) => {
    event.preventDefault();
    const name = form.name.trim();
    if (!name) return setMessage("이름을 입력해주세요.");
    if (tab === "subcategory" && !form.categoryId) return setMessage("대분류를 선택해주세요.");
    setBusy(true); setMessage("");
    try {
      await archiveIndexService.save(tab, { ...form, name, categoryId: Number(form.categoryId || 0), sortOrder: Number(form.sortOrder || 0) });
      await load(); reset(tab); setMessage(form.id ? "수정했습니다." : "추가했습니다.");
    } catch (error) { setMessage(error.message || "저장하지 못했습니다. 같은 이름이 이미 있는지 확인해주세요."); }
    finally { setBusy(false); }
  };
  const remove = async (item) => {
    const childWarning = tab === "category" && item.subcategories?.length ? `\n연결된 소분류 ${item.subcategories.length}개도 함께 삭제됩니다.` : "";
    if (!confirm(`「${item.name}」을(를) 삭제할까요?${childWarning}`)) return;
    setBusy(true); setMessage("");
    try { await archiveIndexService.remove(tab, item.id); await load(); if (String(item.id) === form.id) reset(tab); setMessage("삭제했습니다."); }
    catch (error) { setMessage(error.message || "사용 중인 인덱스는 삭제할 수 없습니다."); }
    finally { setBusy(false); }
  };
  return <div className="pt-5">
    <button onClick={onBack} className="flex items-center gap-1 text-xs font-bold text-neutral-500"><ChevronLeft size={15}/>관리자 선택</button>
    <div className="mt-4 grid grid-cols-3 border-b border-white/10">{[["category","대분류"],["subcategory","소분류"],["keyword","키워드"]].map(([key,label]) => <button key={key} type="button" onClick={()=>changeTab(key)} className={cn("border-b-2 py-3 text-[11px] font-black", tab === key ? "border-accent text-white" : "border-transparent text-neutral-600")}>{label}</button>)}</div>
    <form onSubmit={save} className="mt-4 rounded-2xl border border-white/10 bg-white/[.03] p-4">
      <div className="flex items-center"><h3 className="text-sm font-black">{form.id ? "인덱스 수정" : "새 인덱스 추가"}</h3>{form.id && <button type="button" onClick={()=>reset(tab)} className="ml-auto text-[10px] font-bold text-neutral-500">수정 취소</button>}</div>
      {tab === "subcategory" && <div className="mt-3"><AdminSelect label="소속 대분류" value={form.categoryId} onChange={(value)=>setForm((current)=>({...current,categoryId:value}))} options={data.categories.map((item)=>String(item.id))} optionLabels={Object.fromEntries(data.categories.map((item)=>[String(item.id),item.name]))}/></div>}
      <div className="mt-3 grid grid-cols-[1fr_90px] gap-2"><AdminInput label="이름" value={form.name} onChange={(value)=>setForm((current)=>({...current,name:value}))} required/><AdminInput label="노출 순서" type="number" value={form.sortOrder} onChange={(value)=>setForm((current)=>({...current,sortOrder:value}))}/></div>
      <button disabled={busy || loading} className="mt-3 w-full rounded-xl bg-accent py-3 text-xs font-black disabled:opacity-40">{busy ? "처리 중…" : form.id ? "수정 저장" : "추가"}</button>
    </form>
    {message && <p className="mt-3 rounded-xl border border-white/10 bg-white/[.03] p-3 text-[11px] font-bold text-neutral-400">{message}</p>}
    <div className="mt-5 flex items-center"><h3 className="text-xs font-black">등록된 {tab === "category" ? "대분류" : tab === "subcategory" ? "소분류" : "키워드"}</h3><span className="ml-auto text-[10px] font-bold text-neutral-600">{rows.length}개</span></div>
    {loading ? <div className="mt-3"><ListSkeleton/></div> : tab === "subcategory" ? <div className="mt-3 space-y-3">{data.categories.map((category) => <section key={category.id} className="overflow-hidden rounded-2xl border border-white/10 bg-white/[.02]"><h4 className="border-b border-white/10 bg-white/[.04] px-4 py-3 text-xs font-black">{category.name}</h4><div className="divide-y divide-white/5">{category.subcategories.length ? category.subcategories.map((item) => <div key={item.id} className="flex items-center py-2.5 pl-6 pr-3"><span className="mr-2 text-neutral-600">└</span><div className="min-w-0 flex-1"><p className="truncate text-xs font-bold">{item.name}</p><p className="mt-1 text-[9px] font-bold text-neutral-600">노출 순서 {item.sortOrder}</p></div><button disabled={busy} onClick={()=>edit(item)} className="rounded-lg bg-white/5 p-2 text-neutral-400"><Pencil size={14}/></button><button disabled={busy} onClick={()=>remove(item)} className="ml-1 rounded-lg bg-accent/10 p-2 text-accent"><Trash2 size={14}/></button></div>) : <p className="px-6 py-3 text-[10px] font-bold text-neutral-600">등록된 소분류 없음</p>}</div></section>)}</div> : rows.length ? <div className="mt-3 space-y-2">{rows.map((item) => <div key={item.id} className="flex items-center rounded-xl border border-white/10 bg-white/[.03] p-3"><div className="min-w-0 flex-1"><p className="truncate text-xs font-black">{item.name}</p><p className="mt-1 text-[9px] font-bold text-neutral-600">노출 순서 {item.sortOrder}</p></div><button disabled={busy} onClick={()=>edit(item)} className="rounded-lg bg-white/5 p-2 text-neutral-400"><Pencil size={14}/></button><button disabled={busy} onClick={()=>remove(item)} className="ml-1 rounded-lg bg-accent/10 p-2 text-accent"><Trash2 size={14}/></button></div>)}</div> : <div className="mt-3 rounded-2xl border border-dashed border-white/10 py-10 text-center text-xs font-bold text-neutral-600">등록된 항목이 없습니다.</div>}
  </div>;
}
...
function ArchiveAdmin({ onBack }) {
  ...
  const [monthDayFilter, setMonthDayFilter] = useState(false);
  const [monthDayDate, setMonthDayDate] = useState(adminToday);
  const [selectedMain, setSelectedMain] = useState([]);
  const [bulkDate, setBulkDate] = useState("");
  const [bulkSub, setBulkSub] = useState("");
  const [bulkKeyword, setBulkKeyword] = useState("");
  const [bulkKeywordMode, setBulkKeywordMode] = useState("add");
  ...
  const [rawBulkKeyword, setRawBulkKeyword] = useState("");
  const [rawBulkKeywordMode, setRawBulkKeywordMode] = useState("add");
  const [twitterId, setTwitterId] = useState("");
  const [twitterFrom, setTwitterFrom] = useState("");
  const [twitterTo, setTwitterTo] = useState("");
  const adminCategoryMap = useMemo(() => adminIndex?.categories?.length ? Object.fromEntries(adminIndex.categories.map((category) => [category.name, category.subcategories.map((item) => item.name)])) : CATEGORY_MAP, [adminIndex]);
  const adminSubCategoryOptions = useMemo(() => Object.values(adminCategoryMap).flat(), [adminCategoryMap]);
  const adminMainCategoryFor = (subCategory) => Object.entries(adminCategoryMap).find(([, values]) => values.includes(subCategory))?.[0] || Object.keys(adminCategoryMap)[0] || "기타";
  ...
  const loadList = async (nextPage = page) => {
    ...
    try {
      const result = await archiveService.adminPage({ page: nextPage, limit: 100, query, mainCategory: mainFilter, subCategory: subFilter, dateFrom: monthDayFilter ? "" : dateFrom, dateTo: monthDayFilter ? "" : dateTo, monthDay: monthDayFilter ? monthDayDate.slice(5) : "", status: "published" });
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(timer);
  }, [adminTab, query, mainFilter, subFilter, dateFrom, dateTo, monthDayFilter, monthDayDate]);
  useEffect(() => {
    let active = true;
    Promise.all([keywordService.list(), archiveIndexService.list()]).then(([keywordResult, indexResult]) => {
      if (!active) return;
      setAdminIndex(indexResult);
      setAdminKeywordOptions(uniqueKeywords([...(indexResult.keywords || []).map((item) => item.name), ...(keywordResult.tags || [])]));
    }).catch(() => { if (active) setAdminKeywordOptions([]); });
    return () => { active = false; };
  }, []);
  ...
  const update = (key, value) => { setThumbnailMessage(""); setForm((current) => key === "subCategory" ? { ...current, subCategory: value, mainCategory: adminMainCategoryFor(value) } : { ...current, [key]: value }); };
  ...
  const applyBulk = async (source) => {
    const isRaw = source === "raw"; const targets = isRaw ? selectedRawItems : selectedMainItems; const nextDate = isRaw ? rawBulkDate : bulkDate; const nextSub = isRaw ? rawBulkSub : bulkSub; const nextKeyword = isRaw ? rawBulkKeyword : bulkKeyword; const keywordMode = isRaw ? rawBulkKeywordMode : bulkKeywordMode;
    if (!targets.length) return alert("선택된 항목이 없습니다.");
    if (!nextDate && !nextSub && !nextKeyword) return alert("변경할 날짜, 소분류 또는 키워드를 선택해주세요.");
    if (!confirm(`선택한 ${targets.length}개 항목을 일괄 수정할까요?`)) return;
    setBusy(true);
    const result = await runMany(targets, (item) => { const currentKeywords = uniqueKeywords(item.keywords?.length ? item.keywords : parseKeywordText(item.rawKeywords)); const keywords = !nextKeyword ? currentKeywords : keywordMode === "replace" ? [nextKeyword] : keywordMode === "remove" ? currentKeywords.filter((keyword) => keyword !== nextKeyword) : uniqueKeywords([...currentKeywords, nextKeyword]); const updated = { ...item, date: nextDate || item.date, subCategory: nextSub || item.subCategory, mainCategory: nextSub ? adminMainCategoryFor(nextSub) : item.mainCategory, keywords, rawKeywords: keywords.join(", ") }; return isRaw ? archiveService.updateRaw(item.id, updated) : archiveService.update(item.id, updated); });
    if (isRaw) { await loadRaw(); setRawBulkDate(""); setRawBulkSub(""); setRawBulkKeyword(""); } else { await reloadCurrent(); setSelectedMain([]); setBulkDate(""); setBulkSub(""); setBulkKeyword(""); }
  ...
  const moveMonthDay = (amount) => { const [, month, day] = monthDayDate.split("-").map(Number); const date = new Date(Date.UTC(2000, month - 1, day + amount)); setMonthDayDate(`${adminTodayYear}-${pad(date.getUTCMonth()+1)}-${pad(date.getUTCDate())}`); };
  const resetArchiveFilters = () => { setQuery(""); setMainFilter("전체"); setSubFilter("전체"); setDateFrom(""); setDateTo(""); setMonthDayFilter(false); setMonthDayDate(adminToday); };
  ...
  const bulkPanel = (raw = false) => { const count = raw ? selectedRaw.length : selectedMain.length; const date = raw ? rawBulkDate : bulkDate; const sub = raw ? rawBulkSub : bulkSub; const keyword = raw ? rawBulkKeyword : bulkKeyword; const keywordMode = raw ? rawBulkKeywordMode : bulkKeywordMode; return <div className={cn("mt-3 rounded-2xl border p-3",count ? "border-accent/40 bg-accent/5" : "border-white/10 bg-white/[.02]")}><div className="flex items-center"><p className="text-xs font-black">선택 항목 일괄 수정 <span className="text-accent">{count}개</span></p>{count>0&&<button onClick={()=>raw?setSelectedRaw([]):setSelectedMain([])} className="ml-auto text-[10px] font-bold text-neutral-500">선택 해제</button>}</div><div className="mt-3 grid grid-cols-2 gap-2"><AdminInput label="날짜 변경" type="date" value={date} onChange={raw?setRawBulkDate:setBulkDate}/><GroupedAdminSelect label="소분류 변경" value={sub} onChange={raw?setRawBulkSub:setBulkSub} groups={adminCategoryMap} firstOption="변경 안 함" firstValue=""/></div>{sub&&<p className="mt-2 text-[10px] font-bold text-neutral-500">대분류는 <span className="text-accent">{adminMainCategoryFor(sub)}</span>로 자동 설정됩니다.</p>}<div className="mt-3 grid grid-cols-[110px_1fr] gap-2"><AdminSelect label="키워드 작업" value={keywordMode} onChange={raw?setRawBulkKeywordMode:setBulkKeywordMode} options={["add","remove","replace"]} optionLabels={{add:"추가",remove:"삭제",replace:"전체 교체"}}/><AdminSelect label="키워드 선택" value={keyword} onChange={raw?setRawBulkKeyword:setBulkKeyword} options={["",...adminKeywordOptions]} optionLabels={{"":"변경 안 함"}}/></div><div className="mt-3 grid grid-cols-2 gap-2"><button disabled={!count||busy} onClick={()=>applyBulk(raw?"raw":"main")} className="rounded-xl bg-accent py-2.5 text-xs font-black disabled:bg-neutral-800 disabled:text-neutral-600">선택한 {count}개에 적용</button><button disabled={!count||busy} onClick={()=>refreshSelectedThumbnails(raw?"raw":"main")} className="rounded-xl border border-[#1DA1F2]/40 bg-[#1DA1F2]/10 py-2.5 text-xs font-black text-[#55acee] disabled:opacity-40">썸네일 URL 불러오기</button></div></div>; };
  ...
  return <div className="pt-5">
    ...
    {adminTab === "list" && <><button onClick={() => openForm()} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-accent py-3.5 text-xs font-black"><Plus size={16}/>새 기록 추가</button><div className="mt-3"><SearchBar value={query} onChange={setQuery} placeholder="관리할 기록 검색"/></div><div className="mt-3 grid grid-cols-2 gap-2"><AdminSelect label="대분류 필터" value={mainFilter} onChange={(value)=>{setMainFilter(value);setSubFilter("전체");}} options={["전체",...Object.keys(adminCategoryMap)]}/><GroupedAdminSelect label="소분류 필터" value={subFilter} onChange={(value)=>{setSubFilter(value);if(value!=="전체")setMainFilter(adminMainCategoryFor(value));}} groups={adminCategoryMap} firstOption="전체" firstValue="전체"/><AdminInput label="시작 날짜" type="date" value={dateFrom} onChange={(value)=>{setDateFrom(value);if(value)setMonthDayFilter(false);}} disabled={monthDayFilter}/><AdminInput label="종료 날짜" type="date" value={dateTo} onChange={(value)=>{setDateTo(value);if(value)setMonthDayFilter(false);}} disabled={monthDayFilter}/></div><div className={cn("mt-3 rounded-2xl border p-3",monthDayFilter?"border-accent/40 bg-accent/5":"border-white/10 bg-white/[.02]")}><div className="flex items-center"><div><p className="text-xs font-black">N년전 오늘 필터</p><p className="mt-1 text-[9px] font-bold text-neutral-600">모든 연도의 같은 월·일 기록</p></div><button type="button" onClick={()=>setMonthDayFilter((value)=>!value)} className={cn("ml-auto rounded-full px-3 py-1.5 text-[10px] font-black",monthDayFilter?"bg-accent text-white":"bg-white/5 text-neutral-500")}>{monthDayFilter?"ON":"OFF"}</button></div>{monthDayFilter&&<div className="mt-3 flex items-center gap-2"><button type="button" onClick={()=>moveMonthDay(-1)} className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/5 text-neutral-400"><ChevronLeft size={15}/></button><input type="date" value={monthDayDate} onChange={(event)=>setMonthDayDate(event.target.value||adminToday)} className="h-9 min-w-0 flex-1 rounded-xl border border-white/10 bg-black px-2 text-xs font-bold outline-none"/><button type="button" onClick={()=>moveMonthDay(1)} className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/5 text-neutral-400"><ChevronRight size={15}/></button><button type="button" onClick={()=>setMonthDayDate(adminToday)} className="h-9 rounded-xl bg-white px-3 text-[10px] font-black text-black">오늘</button></div>}</div><div className="mt-3 flex items-center"><label className="flex items-center gap-2 text-[11px] font-bold text-neutral-400"><input type="checkbox" checked={items.length>0&&items.every((item)=>selectedMain.includes(item.id))} onChange={()=>toggleAll(items)} className="h-4 w-4 accent-[#e50000]"/>현재 페이지 전체 선택</label><button type="button" onClick={resetArchiveFilters} className="ml-auto rounded-lg bg-white/5 px-3 py-2 text-[10px] font-black text-neutral-500">필터 초기화</button><button disabled={!selectedMain.length||busy} onClick={removeSelectedMain} className="ml-2 rounded-lg bg-accent/10 px-3 py-2 text-[10px] font-black text-accent disabled:opacity-40">선택 삭제</button></div>{bulkPanel()}<p className="mt-2 text-right text-[10px] font-bold text-neutral-600">검색 결과 {total}개 · {page}페이지</p></>}
    ...
    {formOpen && <form onSubmit={save} className="mt-4 rounded-2xl border border-accent/30 bg-accent/5 p-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <AdminSelect label="대분류" value={form.mainCategory} onChange={(value)=>setForm((current)=>({...current,mainCategory:value,subCategory:adminCategoryMap[value]?.includes(current.subCategory)?current.subCategory:adminCategoryMap[value]?.[0]||""}))} options={Object.keys(adminCategoryMap)}/>
        <GroupedAdminSelect label="소분류" value={form.subCategory} onChange={(v)=>update("subCategory",v)} groups={adminCategoryMap}/>
      </div>
...
function AdminInput({ label, value, onChange, type = "text", required = false, disabled = false }) {
  return <label className="block"><span className="mb-1 ml-1 block text-[9px] font-bold text-neutral-600">{label}</span><input required={required} disabled={disabled} type={type} value={value || ""} onChange={(event)=>onChange(event.target.value)} className="w-full rounded-xl border border-white/10 bg-black px-3 py-2.5 text-xs outline-none focus:border-accent disabled:cursor-not-allowed disabled:opacity-30"/></label>;
}

function AdminSelect({ label, value, onChange, options, emptyValue = false, optionLabels = {} }) {
  return <label className="block min-w-0"><span className="mb-1 ml-1 block text-[9px] font-bold text-neutral-600">{label}</span><select value={value} onChange={(event)=>onChange(event.target.value)} className="w-full rounded-xl border border-white/10 bg-black px-2 py-2.5 text-xs outline-none focus:border-accent">{options.map((option,index)=><option key={`${option}-${index}`} value={emptyValue&&index===0?"":option}>{optionLabels[option] || option}</option>)}</select></label>;
}

function GroupedAdminSelect({ label, value, onChange, groups = {}, firstOption = "", firstValue = "" }) {
  return <label className="block min-w-0"><span className="mb-1 ml-1 block text-[9px] font-bold text-neutral-600">{label}</span><select value={value} onChange={(event)=>onChange(event.target.value)} className="w-full rounded-xl border border-white/10 bg-black px-2 py-2.5 text-xs outline-none focus:border-accent">{firstOption && <option value={firstValue}>{firstOption}</option>}{Object.entries(groups).map(([main, subs]) => <optgroup key={main} label={main}>{subs.map((sub) => <option key={`${main}-${sub}`} value={sub}>└ {sub}</option>)}</optgroup>)}</select></label>;
}
