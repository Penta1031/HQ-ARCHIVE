...
function CalendarView({ items }) {
  ...
  const filtered = day ? monthly.filter((x) => x.date === `${monthKey}-${pad(day)}`) : monthly;
  const firstDay = new Date(year, month - 1, 1).getDay(); const days = new Date(year, month, 0).getDate();
  const changeMonth = (delta) => { const d = new Date(year, month - 1 + delta, 1); setYear(d.getFullYear()); setMonth(d.getMonth() + 1); setDay(null); };
  return <div className="px-4 pt-4"><div className="flex items-center"><div><p className="text-[10px] font-black tracking-[.18em] text-accent">CALENDAR</p><h2 className="mt-1 text-2xl font-black tracking-tight">기록의 시간</h2></div><span className="ml-auto text-sm font-black text-neutral-500">{monthly.length}개</span></div>
    <div className="mt-5 rounded-3xl border border-white/10 bg-neutral-900/70 p-4">
      <div className="flex items-center justify-between"><button onClick={() => changeMonth(-1)} className="rounded-full bg-white/5 p-2"><ChevronLeft size={17} /></button><div className="flex gap-2"><select value={year} onChange={(e) => { setYear(+e.target.value); setDay(null); }} className="rounded-lg bg-black px-2 py-1.5 text-sm font-black outline-none">{[2023,2024,2025,2026].map((y) => <option key={y}>{y}</option>)}</select><select value={month} onChange={(e) => { setMonth(+e.target.value); setDay(null); }} className="rounded-lg bg-black px-2 py-1.5 text-sm font-black outline-none">{Array.from({length:12},(_,i)=>i+1).map((m) => <option key={m} value={m}>{pad(m)}</option>)}</select></div><button onClick={() => changeMonth(1)} className="rounded-full bg-white/5 p-2"><ChevronRight size={17} /></button></div>
...
import {
  Archive, Bookmark, CalendarDays, ChevronLeft, ChevronRight, Crown, Flame, Heart,
  Home, Medal, Play, Search, Share2, Sparkles, Trophy, UserRound, X
} from "lucide-react";
import { archiveItems, globalRanking, keywordGroups, postypeItems, worldcups } from "../lib/data";
...
function Header() {
  return (
    <header className="z-30 flex h-[68px] shrink-0 items-center border-b border-white/10 bg-black/90 px-5 backdrop-blur-xl">
      ...
      <div className="ml-auto rounded-full border border-white/10 bg-white/5 px-2.5 py-1 text-[10px] font-bold text-neutral-500">BETA</div>
    </header>
  );
}
...
function HomeView({ initialKeyword, onKeywordConsumed }) {
  ...
  const [mainCategory, setMainCategory] = useState("전체");
  const [subCategory, setSubCategory] = useState("전체");
  const mains = ["전체", ...new Set(archiveItems.map((x) => x.mainCategory))];
  const subs = ["전체", ...new Set(archiveItems.filter((x) => mainCategory === "전체" || x.mainCategory === mainCategory).map((x) => x.subCategory))];
  ...
  const yearsAgo = archiveItems.filter((x) => x.date.slice(5) === formatDate(today).slice(5) && x.date !== formatDate(today));
...
function ArchiveCard({ item, index = 0 }) {
  return <article className="group min-w-0 animate-fade-in" style={{ animationDelay: `${index * 35}ms` }}>
    <div className="relative aspect-[4/5] overflow-hidden rounded-2xl bg-neutral-900">
      <img src={item.thumbnailUrl} alt="" className="h-full w-full object-cover transition duration-500 group-active:scale-105" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/45 via-transparent to-transparent" />
      <div className="absolute right-2 top-2 rounded-full bg-black/55 p-1.5 backdrop-blur"><Bookmark size={13} /></div>
    </div>
    <div className="mt-2.5 flex items-center gap-2 text-[10px] font-bold"><span className="text-accent">{item.subCategory}</span><time className="ml-auto whitespace-nowrap text-neutral-600">{item.date}</time></div>
...
function CalendarView() {
  const [year, setYear] = useState(2026); const [month, setMonth] = useState(6); const [day, setDay] = useState(null);
  const monthKey = `${year}-${pad(month)}`;
  const monthly = archiveItems.filter((x) => x.date.startsWith(monthKey));
  const filtered = day ? monthly.filter((x) => x.date === `${monthKey}-${pad(day)}`) : monthly;
  const firstDay = new Date(year, month - 1, 1).getDay(); const days = new Date(year, month, 0).getDate();
  const changeMonth = (delta) => { const d = new Date(year, month - 1 + delta, 1); setYear(d.getFullYear()); setMonth(d.getMonth() + 1); setDay(null); };
  return <div className="px-4 pt-4"><div className="flex items-center"><div><p className="text-[10px] font-black tracking-[.18em] text-accent">CALENDAR</p><h2 className="mt-1 text-2xl font-black tracking-tight">기록의 시간</h2></div><span className="ml-auto text-sm font-black text-neutral-500">{monthly.length}개</span></div>
