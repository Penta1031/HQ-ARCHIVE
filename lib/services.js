...
export const recommendedVideoService = {
  async categories() {
    const result = await supabaseRequest("recommended_video_categories?select=id,name,sort_order&order=sort_order.asc,created_at.asc");
  ...
  async createCategory(name) {
    const result = await adminRequest("recommended-video-category-create", { name });
    return mapRecommendedVideoCategory(result.category);
  },
  async updateCategory(id, name) {
    const result = await adminRequest("recommended-video-category-update", { id, name });
    return mapRecommendedVideoCategory(result.category);
  },
