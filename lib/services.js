...
export const recommendedVideoService = {
  async publicList() {
    const fields = "id,youtube_url,title,published_at,thumbnail_url,categories,sort_order";
    const result = await supabaseRequest(`recommended_videos?select=${fields}&is_active=eq.true&order=sort_order.asc,published_at.desc.nullslast&limit=1000`);
    return result.rows.map(mapRecommendedVideo);
  },
  async featuredList() {
    const fields = "id,youtube_url,title,published_at,thumbnail_url,categories,admin_comment,featured_order";
  ...
  async list() {
    const result = await adminRequest("recommended-video-list");
    return result.items.map(mapRecommendedVideo);
  },
  ...
  async collectChannels() {
    return adminRequest("recommended-video-channel-sync");
  }
};
