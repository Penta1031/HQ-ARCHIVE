...
const WORLDCUP_FIREBASE = {
  apiKey: "AIzaSyDLCJ2RXCl6u-oPicWPjRo8K5in_xmkotg",
  projectId: "hqworldcup-46819"
};

const worldcupCollectionUrl = `https://firestore.googleapis.com/v1/projects/${WORLDCUP_FIREBASE.projectId}/databases/(default)/documents/worldcups`;
let worldcupAuthPromise;
...
const decodeFirestoreValue = (value = {}) => {
  ...
  if (value.mapValue) return Object.fromEntries(Object.entries(value.mapValue.fields || {}).map(([key, nested]) => [key, decodeFirestoreValue(nested)]));
  return undefined;
};
...
const encodeFirestoreValue = (value) => {
  ...
  if (typeof value === "object") return { mapValue: { fields: Object.fromEntries(Object.entries(value).filter(([, nested]) => nested !== undefined).map(([key, nested]) => [key, encodeFirestoreValue(nested)])) } };
  return { stringValue: String(value) };
};

const normalizeWorldcupItem = (item = {}) => ({
  id: item.id ?? Date.now(),
  name: String(item.name || item.title || "").trim(),
  date: toDate(item.date),
  sourceUrl: String(item.sourceUrl || "").trim(),
  mediaUrl: String(item.mediaUrl || item.thumbnailUrl || "").trim(),
  isVideo: Boolean(item.isVideo || /\.mp4(?:$|\?)/i.test(item.mediaUrl || "")),
  winCount: Number(item.winCount || 0)
});

const normalizeWorldcup = (cup = {}) => ({
  id: cup.id ?? Date.now(),
  title: String(cup.title || "새로운 혚쾌 월드컵").trim(),
  icon: String(cup.icon || "🏆").trim() || "🏆",
  items: (cup.items || []).map(normalizeWorldcupItem)
});

async function getWorldcupIdToken() {
  if (!worldcupAuthPromise) {
    worldcupAuthPromise = (async () => {
      if (typeof window !== "undefined") {
        const cached = window.sessionStorage.getItem("hq-worldcup-auth");
        if (cached) {
          try {
            const parsed = JSON.parse(cached);
            if (parsed.idToken && parsed.expiresAt > Date.now() + 60_000) return parsed.idToken;
          } catch {}
        }
      }
      const response = await fetch(`https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=${WORLDCUP_FIREBASE.apiKey}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ returnSecureToken: true })
      });
      const result = await response.json();
      if (!response.ok || !result.idToken) throw new Error(result.error?.message || "월드컵 인증에 실패했습니다.");
      if (typeof window !== "undefined") window.sessionStorage.setItem("hq-worldcup-auth", JSON.stringify({ idToken: result.idToken, expiresAt: Date.now() + Number(result.expiresIn || 3600) * 1000 }));
      return result.idToken;
    })().catch((error) => { worldcupAuthPromise = null; throw error; });
  }
  return worldcupAuthPromise;
}

async function worldcupRequest(url, options = {}) {
  const token = await getWorldcupIdToken();
  ...
  if (!response.ok) {
    ...
    throw new Error(result.error?.message || `월드컵 데이터 연결 오류 (${response.status})`);
  }
...
export const worldcupService = {
  async list() {
    const result = await worldcupRequest(`${worldcupCollectionUrl}?pageSize=100`);
    return (result.documents || []).map((document) => normalizeWorldcup(Object.fromEntries(Object.entries(document.fields || {}).map(([key, value]) => [key, decodeFirestoreValue(value)])))).sort((a, b) => Number(a.id) - Number(b.id));
  },
  async save(cup) {
    const normalized = normalizeWorldcup(cup);
    const fields = Object.fromEntries(Object.entries(normalized).map(([key, value]) => [key, encodeFirestoreValue(value)]));
    await worldcupRequest(`${worldcupCollectionUrl}/${encodeURIComponent(normalized.id)}`, { method: "PATCH", body: JSON.stringify({ fields }) });
    return normalized;
  },
  async remove(id) {
    await worldcupRequest(`${worldcupCollectionUrl}/${encodeURIComponent(id)}`, { method: "DELETE" });
    return true;
  }
};
