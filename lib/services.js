...
export { adminService, analyticsService, appTabService, archiveIndexService, archiveService, keywordService, tweetMediaService } from "./archive-supabase";
...
export const recommendedVideoService = {
  ...
  async list({ offset = 0, limit = 30, query = "", category = "", isActive = "all", isFeatured = "all", isHyeopkwaePick = "all" } = {}) {
    const result = await adminRequest("recommended-video-list", { offset, limit, query, category, isActive, isFeatured, isHyeopkwaePick });
    return {
      items: result.items.map(mapRecommendedVideo),
      total: Number(result.total || 0),
      hasMore: Boolean(result.hasMore)
    };
  },
  async adminSummary() {
    const result = await adminRequest("recommended-video-admin-summary");
    return {
      total: Number(result.total || 0),
      featuredActiveCount: Number(result.featuredActiveCount || 0),
      hyeopkwaePickActiveCount: Number(result.hyeopkwaePickActiveCount || 0),
      featuredItems: (result.featuredItems || []).map(mapRecommendedVideo)
    };
  },
  ...
  async refreshPublishedDates() {
    return adminRequest("recommended-video-refresh-published-dates");
  }
};
