...
const mapArchiveIndex = (categoryRows = [], subcategoryRows = [], keywordRows = []) => ({
  categories: categoryRows.map((row) => ({
    id: Number(row.id), name: clean(row.name), sortOrder: Number(row.sort_order || 0),
    subcategories: subcategoryRows.filter((sub) => Number(sub.category_id) === Number(row.id)).map((sub) => ({ id: Number(sub.id), categoryId: Number(sub.category_id), name: clean(sub.name), sortOrder: Number(sub.sort_order || 0) }))
  })),
  keywords: keywordRows.map((row) => ({ id: Number(row.id), name: clean(row.name), sortOrder: Number(row.sort_order || 0) }))
});

export const archiveIndexService = {
  async list() {
    const [categories, subcategories, keywords] = await Promise.all([
      supabaseRequest("hq_archive_categories?select=id,name,sort_order&is_active=eq.true&order=sort_order.asc,id.asc"),
      supabaseRequest("hq_archive_subcategories?select=id,category_id,name,sort_order&is_active=eq.true&order=sort_order.asc,id.asc"),
      supabaseRequest("hq_archive_keywords?select=id,name,sort_order&is_active=eq.true&order=sort_order.asc,id.asc")
    ]);
    return mapArchiveIndex(categories.rows, subcategories.rows, keywords.rows);
  },
  async save(kind, item) { return adminRequest("archive-index-save", { kind, item }); },
  async remove(kind, id) { return adminRequest("archive-index-delete", { kind, id }); }
};
...
export const analyticsService = {
  async track({ eventType = "tab", tabKey = "home", contentType = "", contentId = "", contentTitle = "", contentUrl = "" } = {}) {
    ...
    const row = {
      event_type: type,
      tab_key: ["home", "calendar", "recommended"].includes(tabKey) ? tabKey : "home",
  ...
  async trackSearch({ query = "", queryKind = "title", resultCount = 0, tabKey = "home" } = {}) {
    if (typeof window === "undefined" || !String(query || "").trim()) return false;
    const row = {
      query_text: trimForStats(String(query).trim(), 300),
      query_kind: ["title", "keyword", "date"].includes(queryKind) ? queryKind : "title",
    ...
    try {
      const response = await fetch(`${SUPABASE_URL}/rest/v1/hq_search_events`, {
        method: "POST",
  ...
  async searchStats({ dateFrom = "", dateTo = "", limit = 20 } = {}) {
    const result = await adminRequest("search-stats", { dateFrom, dateTo, limit });
    return {
      dateFrom: result.dateFrom || dateFrom,
      dateTo: result.dateTo || dateTo,
      totals: result.totals || { searches: 0, visitors: 0, zeroResults: 0 },
      top: result.top || [],
